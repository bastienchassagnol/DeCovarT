...
plot_correlation_Heatmap <- function(distribution_metrics, score_variable = "model_mse",
  ...
  distribution_metrics <- distribution_metrics %>% 
    ...
    dplyr::mutate(algorithm=factor(algorithm, levels = unique(algorithm)))
  
  # design the scaling color
  mean_distribution_metrics <- distribution_metrics %>% 
    dplyr::group_by(correlation_celltype1, correlation_celltype2, algorithm) %>% 
  ...
  if (uni_scale) col <- circlize::colorRamp2(seq(min_metric,  max_metric, length.out = n_break), 
