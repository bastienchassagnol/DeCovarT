...
j},\boldsymbol{\Sigma}\_{j})\\. If `p` is omitted it defaults to the
equi-balanced vector \\1/J\\, which recovers the uniform pairwise
average \\2/(J(J-1))\sum\_{j\<\ell}J(f\_{j},f\_{\ell})\\.
...
- true_theta:
  ...
  List with `mu` (\\G\times J\\), `sigma` (\\G\times G\times J\\), and
  optionally `p` (length \\J\\). If `p` is missing it is set to
  \\(1/J,\ldots,1/J)\\.

- J:

  Number of cell types. Defaults to `length(p)` after the equi-balanced
  default is applied.
