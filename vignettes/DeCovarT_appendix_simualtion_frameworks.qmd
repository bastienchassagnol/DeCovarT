...
is full, cell-type diagonal, or a **fixed** GLS covariance, and the
effect of random ILR starts on second-order solvers. CPM / log2
normalisation, optimiser tolerance, and scaling with $G$, $J$ and
overlap are left to a later benchmark issue.
...
3. Fixed global diagonal GLS: $W=\operatorname{diag}\{\Sigma(p^{\star})\}$
   held **constant**, fitted by [`deconvolute_ratios_gls()`](../reference/deconvolute_ratios_gls.html)
...
several draws (or [`multistart_decovart()`](../reference/multistart_decovart.html))
rather than a single random start. A QP warm start is
`initial_p = "qp"`. On this well-conditioned toy the second-order
estimates should agree; a spread of several percentage points is a
warning that the start has trapped the solver.
