...
checked against **numDeriv** [@R-numDeriv]), and
...
**compositions** package [@R-compositions] exposes `alr()` / `alrInv()`.
DeCovarT's `additive_logistic()` and `additive_log_ratio()` are the same maps with the last
