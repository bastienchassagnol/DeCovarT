---
title: "Appendix S4 — Covariance Modelling Assumptions"
subtitle: "Full script: `scripts/supp_S4_covariance_modeling.R`"
vignette: >
  %\VignetteIndexEntry{Appendix S4 — Covariance Modelling Assumptions}
...
> **Script:** `scripts/supp_S4_covariance_modeling.R` — compares covariance
> modelling assumptions.  Outputs in `output/supp_S4/`.
...
The DeCovarT likelihood requires specifying
$\boldsymbol{\Sigma}_j$ — the within-cell-type covariance.  In practice,
these are estimated from external (reference) data and may be mis-specified.
This appendix compares four **inference models** that differ in how
...
non-oracular, matching the standard practice when only pooled variability
is known.
...
$G = 50$ genes, $J = 3$ cell types; $N = 200$ Monte Carlo replicates
(smoke test: $n = 2$).
...
See details of the gls competitor in @nte-gls-solver.


::: {#nte-gls-solver .callout-important}
## The fixed-covariance GLS (generalized least squares) solver
...
- The **RMSE gap** (true full minus alternatives) should increase with
  cosine $\rho_\mu$, because near-collinear means force greater reliance on
  the covariance for discrimination.
...
- [Appendix S5 — Model Mis-specification](https://bastienchassagnol.github.io/DeCovarT/articles/supp-S5-misspecification.html)
