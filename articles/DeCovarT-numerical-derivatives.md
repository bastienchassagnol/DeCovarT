...
DeCovarT estimates the cellular ratios \boldsymbol{p}\in\Delta^{J-1} of
a bulk sample by maximising a log-likelihood over an *unconstrained*
parameter \boldsymbol{\rho}\in\mathbb{R}^{J-1}. Two reciprocal maps move
between the two representations, and both are standard transforms from
...
[Eq. 1](#eq-additive-logistic) is a softmax in which the last category J
is pinned as a reference (\rho_J\equiv 0); in Aitchison’s framework it
is the *additive logistic transform*, i.e. the inverse additive
log-ratio map (\mathrm{alr}^{-1}).
