...
vignette](https://bastienchassagnol.github.io/DeCovarT/articles/generative-model-derivatives.md).
...
**Proposition 1 (Identifiability of the composition)** Two compositions
induce the same law \mathcal{N}\_G(\boldsymbol{\mu}\boldsymbol{p},
...
\boldsymbol{\mu}\_{\cdot j}-\boldsymbol{\mu}\_{\cdot J}, j=1,\ldots,J-1,
to be linearly independent, i.e. \operatorname{rank}(\boldsymbol{\mu})=J
with G\ge J in the generic case.
...
[feature-selection
vignette](https://bastienchassagnol.github.io/DeCovarT/articles/feature-selection.html#sec-weighted-kappa)).
...
and studied numerically in the [appendix
simulations](https://bastienchassagnol.github.io/DeCovarT/articles/DeCovarT_appendix_simualtion_frameworks.md).

### Label permutation is not a bootstrap

[Proposition 1](#prp-identifiability) identifies \boldsymbol{p} *given a
labelled reference*. It does not identify the labels themselves. Write
...
\boldsymbol{\mu}\boldsymbol{p}, \qquad
\boldsymbol{\Sigma}(\boldsymbol{p}\_{\pi};\boldsymbol{\Sigma}\_{\pi}) =
\boldsymbol{\Sigma}(\boldsymbol{p}). \tag{2}

**Theorem 1 (Equivariance under cell-type relabelling)** For every
permutation \pi of the cell types and every bulk profile \boldsymbol{y},
...
identifiable **only up to labelling**. Ordinary K-component mixtures are
typically identified up to permutation of the component labels, which is
distinct from degeneracies such as coincident components or zero mixing
weights ([Xiao et al.
2025](#ref-xiaoBayesianIdentificationEstimation2025)). Under an
exchangeable prior the posterior inherits that symmetry: there are up to
K! permutation-equivalent representations, and component-specific
labelled summaries are meaningless until a relabelling convention is
imposed ([Grün and Malsiner-Walli
2022](#ref-grunBayesianFiniteMixture2022); [Newman
2025](#ref-newmanFastSamplingModel2025)). The canonical frequentist and
MCMC accounts are Stephens
([2000](#ref-stephensDealingLabelSwitching2000)), who showed that
artificial identifiability constraints do not in general solve the
inferential problem, Jasra et al.
([2005](#ref-jasraMarkovChainMonte2005)) for the MCMC review, and
Papastamoulis ([2016](#ref-papastamoulisLabelSwitchingPackage2016)) for
systematic post-processing.
...
proportions must be inferred ([Avila Cobos et al.
2018](#ref-avilacobosComputationalDeconvolutionTranscriptomics2018);
[Nguyen et al. 2024](#ref-nguyenFourteenYearsCellular2024); [Sturm et
al. 2019](#ref-sturmComprehensiveEvaluationTranscriptomebased2019);
[Gaspard-Boulinc et al.
2025](#ref-gaspard-boulincCelltypeDeconvolutionMethods2025)). Writing
Y\approx SP with a known, named S already anchors “B cell”, “monocyte”,
and so on. Permuting the columns of S together with the corresponding
rows of P is algebraically harmless; permuting P alone, or shuffling the
names of an already averaged reference X, changes the biological meaning
of the fit and is not a valid bootstrap.
...
[`reference_bootstrap_decovart()`](https://bastienchassagnol.github.io/DeCovarT/reference/reference_bootstrap_decovart.md)
resamples donors or cells rather than names
([Sec. 2.6](#sec-reference-bootstrap)).

True label switching returns if *both* S and P are unknown: then
SP=(SQ)(Q^{-1}P) for every permutation matrix Q, and component 1 has no
intrinsic biological meaning across bootstrap or MCMC runs. The remedy
...
failure: if S\_{\cdot A}\approx S\_{\cdot B}, many splits with p_A+p_B
fixed produce almost the same bulk, so individual fractions wander while
their sum is stable. Relabelling does not cure that weak identification
([Sec. 1.1](#sec-identifiability); the [appendix
simulations](https://bastienchassagnol.github.io/DeCovarT/articles/DeCovarT_appendix_simualtion_frameworks.md)
...
Table 1: Reference-based resampling versus label or gene permutation.
Only the first three rows estimate uncertainty; label permutation is a
software check.
...
> The same conclusion transfers to the additive log-ratio chart, because
> a C^{2} diffeomorphism preserves the existence of directions of
> positive curvature. Consequently:
>
...
**Theorem 2 (Population uniqueness through the Kullback–Leibler
divergence)**
...
simulations](https://bastienchassagnol.github.io/DeCovarT/articles/DeCovarT_appendix_simualtion_frameworks.html#sec-affine).
...
[`confint.decovart_fit()`](https://bastienchassagnol.github.io/DeCovarT/reference/fit_decovart.md).
It costs one matrix inversion, but it linearises the additive log-ratio
...
**Theorem 3 (Boundary calibration)** With q constraints active on the
...
> 2.  **Do not encode the null in ALR coordinates.** p_j=0 corresponds
>     to \rho_j=-\infty, so the chart cannot represent the hypothesis.
>     The restricted fits substitute the constrained value directly.
...
[`equivariance_check_decovart()`](https://bastienchassagnol.github.io/DeCovarT/reference/equivariance_check_decovart.md)
is the software counterpart of [Theorem 1](#thm-label-equivariance):
permute the signature columns, refit, and confirm that
\hat{\boldsymbol{p}} merely relabels. It is not the distribution used
...
the convolution mapped through the ALR ([Sec. 2.2](#sec-interior); the
...
| `local_maximum` (small ALR score norm, negative \lambda\_{\max}(\mathbf{H}\_{\boldsymbol{\rho}})) | stationarity with the right local curvature | global optimality |
...
Table 2: What each DeCovarT convergence field does and does not certify.
...
#> sample_1         0.2159583         FALSE 1.456636e-09      -8.272874
...
distinction of [Table 2](#tbl-diagnostics) doing its job rather than a
...
approximations. [Table 3](#tbl-inference-recap) is the practical
...
|:---|:---|:---|:---|:---|:---|
| Wald (ALR delta method) | per-sample CI | \mathrm{N}(\boldsymbol{0},I(\hat{\boldsymbol{\rho}})^{-1}) mapped by \mathbf{J}\_{\boldsymbol{\psi}} | unusable (`NA`) | one bulk column is a plug-in approximation ([Note 3](#nte-replication)) | [`confint()`](https://rdrr.io/r/stats/confint.html) / [`vcov_alr_delta()`](https://bastienchassagnol.github.io/DeCovarT/reference/vcov_alr_delta.md) |
...
Table 3: Inferential procedures implemented for DeCovarT proportions.
...
3.  If the dominant uncertainty is the purified reference, use
    ...
    [Theorem 1](#thm-label-equivariance) makes a label shuffle vacuous,
    and a gene shuffle is not a sampling model
    ([Table 1](#tbl-reference-ops)). Use
    [`equivariance_check_decovart()`](https://bastienchassagnol.github.io/DeCovarT/reference/equivariance_check_decovart.md)
...
>   stays proper where the ALR Wald interval degenerates, and credible
...
>   vignette](https://bastienchassagnol.github.io/DeCovarT/articles/decovart-perspectives.md).
...
Grün, Bettina, and Gertraud Malsiner-Walli. 2022. ‘Bayesian Finite
Mixture Models’. In *Wiley StatsRef: Statistics Reference Online*.
Wiley. <https://doi.org/10.1002/9781118445112.stat08373>.
...
Modeling’. *Statistical Science* 20 (1).
...
Newman, M. E. J. 2025. ‘Fast Sampling and Model Selection for Bayesian
Mixture Models’. *Statistics and Computing* 36 (1).
<https://doi.org/10.1007/s11222-025-10753-0>.
...
Statistical Software* 69. <https://doi.org/10.18637/jss.v069.c01>.
...
Methodology* 62 (4): 795–809. <https://doi.org/10.1111/1467-9868.00265>.
...
Xiao, Xingyao, Sophia Rabe-Hesketh, and Anders Skrondal. 2025. ‘Bayesian
Identification and Estimation of Growth Mixture Models’. *Psychometrika*
90 (2): 442–75. <https://doi.org/10.1017/psy.2025.11>.
