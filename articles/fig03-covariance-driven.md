...
that report the share of variability on the first two coordinates, and a
cowplot scree inset ([drawing on
plots](https://wilkelab.org/cowplot/articles/drawing_with_on_plots.html#making-inset-plots))
of the first ten scaled eigenvalues. The layout follows the `fviz_pca`
style (points, ellipses, percentage axes) without importing
`factoextra`.
