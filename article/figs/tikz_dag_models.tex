...
\begin{tikzpicture}[x=1.2cm, y=1.15cm, >=stealth]
  \node[param] (p) at (0, 1.2) {$\boldsymbol{p}_{i}$};
  \node[param] (sig) at (0, 0) {$\sigma_{i}$};
  % Wider gutters: left (params → gene plate) and right (gene plate → $x_{gj}$)
  \node[obs] (y) at (3.0, 0.6) {$\boldsymbol{y}_{gi}$};
  \node[param] (x) at (6.2, 0.6) {$x_{gj}$};
  ...
  (ylik) {%
    $\boldsymbol{y}_{gi\,|\,\cdot}
    \sim\mathcal{N}\!\bigl(\sum_{j}x_{gj}p_{ji},\,\sigma_{i}^{2}\bigr)$%
  };
...
\begin{tikzpicture}[x=1.1cm, y=1.05cm, >=stealth, font=\small]
  \node[detcirc] (p) at (0, 0.9) {$\boldsymbol{p}_{i}$};
  \node[obsdashed] (y) at (0, -0.6) {$\boldsymbol{y}_{i}$};
  \draw[->, dashed] (p) -- (y);

  \node[latent] (xj) at (4.2, 0.9) {$\boldsymbol{x}_{j}$};
  \node[param] (mu) at (3.3, 2.3) {$\boldsymbol{\mu}_{j}$};
  ...
  {$\boldsymbol{y}_{i}=\boldsymbol{\mu}\boldsymbol{p}_{i}$};
...
\begin{tikzpicture}[x=1.1cm, y=1.0cm, >=stealth, font=\small]
  \node[latent] (th) at (0, 2.1) {$\boldsymbol{\theta}_{i}$};
  \node[detcirc] (p) at (0, 0.9) {$\boldsymbol{p}_{i}$};
  \node[obsdashed] (y) at (0, -0.55) {$\boldsymbol{y}_{i}$};
  \draw[->] (th) -- (p);
  \draw[->, dashed] (p) -- (y);
