...
\begin{document}
    ...
    \subsubsection{Bivariate toy: two genes and two cell types}
    \label{sec:bivariate-toy}

    To verify the benefit of modelling gene--gene covariance, we
    designed a minimal scenario: $G=2$ genes and $J=2$ cell types.
    The single free ILR coordinate $z_1\in\mathbb{R}$ is mapped to
    the open simplex via (\ref{eq-mapping-function}), so the likelihood
    is one-dimensional and directly inspectable.

    We generated $N=500$ bulk replicates under the following factorial
    design (972 scenarios in total; code in
    \texttt{scripts/fig02\_bivariate\_toy.R}):

    \begin{itemize}
        \item \textbf{Composition:} balanced $\boldsymbol{p}=(1/2,1/2)$,
            moderately unbalanced $\boldsymbol{p}=(17/20,3/20)$,
            or highly unbalanced $\boldsymbol{p}=(99/100,1/100)$.

        \item \textbf{Mean centroids (CLD):} small separation,
            $\boldsymbol{\mu}_{\cdot 1}=(20,22)$ and
            $\boldsymbol{\mu}_{\cdot 2}=(22,20)$; or large separation,
            $\boldsymbol{\mu}_{\cdot 1}=(20,40)$ and
            $\boldsymbol{\mu}_{\cdot 2}=(40,20)$.

        \item \textbf{Gene--gene correlation:} the within-cell-type
            correlation $\rho_{j}\in\{-0.8,-0.6,\ldots,0.8\}$ (nine
            levels) was set independently for each of the two cell types.

        \item \textbf{Variance structure:} homoscedastic
            ($\sigma^{2}=1$ for both genes) or heteroscedastic
            ($\sigma_{1}^{2}=1$, $\sigma_{2}^{2}=2$).
    \end{itemize}

    Seven solvers were benchmarked: NNLS and DeconRNASeq
    (mean-only baselines), L-BFGS-B, projected gradient descent,
    Newton--Raphson, Marquardt--Levenberg, and simulated annealing.
    DeConRNASeq implements NNLS via the \texttt{lsei} package
    \cite{gongDeconRNASeqStatisticalFramework2013}.
    Performance was summarised by the ADEMP framework
    \cite{morrisUsingSimulationStudies2019} (bias, RMSE, coverage with
    Wilson intervals, and optimiser failure rate).

    We compared the performance of DeCovarT with DeconRNASeq
    \cite{gongDeconRNASeqStatisticalFramework2013}.
    ...
    \subsubsection{High-dimensional hybrid scenario: $G=50$ genes and three cell types}
    \label{sec:hybrid-scenario}

    The bivariate toy isolates the effect of gene--gene covariance in a
    fully transparent setting but cannot demonstrate scalability or the
    realistic topology of gene-regulatory networks (GRNs).  We therefore
    designed a second scenario with $G=50$ genes and $J=3$ cell types
    (code in \texttt{scripts/fig03\_hybrid\_scenario.R};
    detailed parameter tables in the package vignette
    \texttt{fig02-bivariate-toy} / \texttt{fig03-variance-driven}).

    \paragraph{Generative model.}
    The mean matrix $\boldsymbol{\mu}\in\mathbb{R}^{50\times 3}$ is
    built from three gene blocks:
    (i)~30 \emph{shared} genes with high pairwise cosine between cell
    types~1 and~2 ($\rho_{12}\approx 0.97$); these genes carry no
    discriminative mean signal between the two types;
    (ii)~10 \emph{marker} genes highly expressed only in cell type~3
    ($\rho\approx 0.56$ with the shared block); and
    (iii)~10 \emph{null} genes equally expressed across all three types.
    Cell-type-specific precision matrices $\boldsymbol{\Omega}_{j}$ are
    block-diagonal, with each block sampled from a distinct random-graph
    model: stochastic block model (hub structure, cell type~1), star graph
    (one regulatory driver, cell type~2), and Erd\H{o}s--R\'enyi (cell
    type~3 and all null-gene blocks).  Precision matrices are shifted to
    be positive-definite following (\ref{eq-affine-spectrum-transformation})
    \cite{chiquetVariationalInferenceSparse2018}.

    \paragraph{Inference.}
    Three solvers were compared on $N=200$ simulated bulks at three
    composition levels (balanced, moderately unbalanced, and highly
    unbalanced): NNLS, DeconRNASeq
    \cite{gongDeconRNASeqStatisticalFramework2013}, and DeCovarT
    (Marquardt--Levenberg).  Because cell types~1 and~2 are near
    mean-collinear ($\rho_{12}\approx 0.97$), mean-only methods
    cannot separate them; the network topology encoded in
    $\boldsymbol{\Sigma}_{j}$ is the only discriminative signal available
    to DeCovarT.  Results are reported via the ADEMP operating
    characteristics \cite{morrisUsingSimulationStudies2019}.

    \paragraph{GRN construction.}
    Random adjacency matrices were produced by
    \texttt{generate\_random\_network\_skeleton()} and converted to
    positive-definite precision matrices by
    \texttt{build\_normalised\_precision()}, which adds a diagonal shift
    $u\boldsymbol{I}_{G}$ to ensure strict positive-definiteness
    (Appendix~\ref{supp-thm-affine}).  Alternative GRN generators---Toeplitz
    covariances (one-parameter, encoding local dependencies only) and
    the \texttt{MixSim} overlap-based framework
    \cite{melnykovMixSimPackageSimulating2012}---were considered but not
    adopted: neither models cell-type-specific sparse precision structure.
    ...
    \begin{itemize}
        ...
        \item $\boldsymbol{\Sigma}_{j}\in\mathbb{R}^{G\times G}$ and
            $\boldsymbol{\Omega}_{j}\equiv\boldsymbol{\Sigma}_{j}^{-1}$:
    ...
    \texttt{theory-related-models-beyond-OLS}.
    ...
    \begin{equation}
        ...
        \begin{split}
            ...
            p_{j}}=&\scriptstyle \frac{1}{2}\frac{\partial \log\left(\DET(\boldsymbol{\Omega})\right)}{\partial
            p_{j}}-\frac{1}{2}\left[\frac{\partial (\boldsymbol{y}- \boldsymbol{\mu}\boldsymbol{p})^{\top}}{\partial
            p_{j}}\boldsymbol{\Omega}(\boldsymbol{y}- \boldsymbol{\mu}\boldsymbol
            {p}) + (\boldsymbol{y}- \boldsymbol{\mu}\boldsymbol{p})^{\top}\frac{\partial\boldsymbol{\Omega}}{\partial
            p_{j}}(\boldsymbol{y}- \boldsymbol{\mu}\boldsymbol{p}) + (\boldsymbol
            {y}- \boldsymbol{\mu}\boldsymbol{p})^{\top}\boldsymbol{\Omega}\frac{\partial
            (\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})}{\partial p_{j}}\right
            ]\\
            =&\scriptstyle -\frac{1}{2}\Tr \left(\boldsymbol{\Omega}\frac{\partial
            \boldsymbol{\Sigma}}{\partial p_{j}}\right) - \frac{1}{2}\left[ - \boldsymbol
            {\mu}_{\cdot j}^{\top}\boldsymbol{\Omega}(\boldsymbol{y}- \boldsymbol{\mu}
            \boldsymbol{p}) - (\boldsymbol{y}- \boldsymbol{\mu}\boldsymbol{p})^{\top}
            \Omega\frac{\partial \Sigma}{\partial p_{j}}\Omega(\boldsymbol{y}- \boldsymbol
            {\mu}\boldsymbol{p}) - (\boldsymbol{y}- \boldsymbol{\mu}\boldsymbol{p}
            )^{\top}\boldsymbol{\Omega}\boldsymbol{\mu}_{\cdot j}\right] \\
            =&\textcolor{purple}{-p_j \Tr \left(\boldsymbol{\Omega}\boldsymbol{\Sigma}_j\right)
            }+ \textcolor{blue}{(\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})^\top\boldsymbol{\Omega}
            \boldsymbol{\mu}_{\cdot j}}\, + \textcolor{orange}{p_j (\boldsymbol{y} -
            \boldsymbol{\mu} \boldsymbol{p})^\top\boldsymbol{\Omega} \Sigma_j
            \boldsymbol{\Omega} (\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})
            }
        \end{split}
    \end{equation}
    ...
    \begin{equation}
        ...
        \begin{aligned}
            \mathbf{H}_{i,i} & = \frac{\partial^2 \ell}{\partial^2 p_i}= \textcolor{purple}{-\Tr \left(\boldsymbol{\Omega}\boldsymbol{\Sigma}_i\right) + 2p_i^2 \Tr \left(\left(\boldsymbol{\Omega}\boldsymbol{\Sigma}_i\right)^2\right)}\textcolor{blue}{-2p_i(\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})^\top\boldsymbol{\Omega} \boldsymbol{\Sigma}_i \boldsymbol{\Omega} \boldsymbol{\mu}_{\cdot i}\, - \boldsymbol{\mu}_{\cdot i}^\top\boldsymbol{\Omega} \boldsymbol{\mu}_{\cdot i}}\, -        \\
                             & \textcolor{orange}{2p_i (\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})^\top \boldsymbol{\Omega}\boldsymbol{\Sigma}_i\boldsymbol{\Omega}\boldsymbol{\mu}_{\cdot i} \, - (\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})^\top\boldsymbol{\Omega} \left(4p_i^2 \boldsymbol{\Sigma}_i \boldsymbol{\Omega} \boldsymbol{\Sigma}_i - \boldsymbol{\Sigma}_i \right)\boldsymbol{\Omega} (\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})}, \quad i \in \widetilde{J} \\
            \mathbf{H}_{i,j} & = \frac{\partial^2 \ell}{\partial p_i \partial p_j}= \textcolor{purple}{2p_j p_i \Tr \left(\boldsymbol{\Omega}\boldsymbol{\Sigma}_j \boldsymbol{\Omega}\boldsymbol{\Sigma}_i \right)}\textcolor{blue}{-2p_i(\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})^\top\boldsymbol{\Omega} \boldsymbol{\Sigma}_i \boldsymbol{\Omega} \boldsymbol{\mu}_{\cdot j} - \boldsymbol{\mu}_{\cdot i}^\top\boldsymbol{\Omega} \boldsymbol{\mu}_{\cdot j}}\, -                                \\
                             & \textcolor{orange}{2p_j (\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})^\top \boldsymbol{\Omega}\boldsymbol{\Sigma}_j\boldsymbol{\Omega} \boldsymbol{\mu}_{\cdot i}\, - 4p_ip_j(\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})^\top\boldsymbol{\Omega}\boldsymbol{\Sigma}_i \boldsymbol{\Omega} \boldsymbol{\Sigma}_j \boldsymbol{\Omega} (\boldsymbol{y} - \boldsymbol{\mu} \boldsymbol{p})}, \quad (i,j) \in \widetilde{J}^{2}, i \neq j
        \end{aligned}
    \end{equation}
    ...
    $\boldsymbol{\Omega}_{j}$ additionally shrink non-zero partial correlations.
    ...
    \begin{appendices}
        ...
        % vignettes/theory-related-models-beyond-OLS.qmd (kept out of the Article word budget).
        ...
        \begin{definition}[Multivariate Gaussian distributions]
            ...
            \begin{itemize}
                ...
                \item $\boldsymbol{\Sigma}$ is a $G\times G$ positive-definite \ref{def:positive-definite}
                    ...
                    are the covariance between variables. We note $\Omega=\Sigma^{-1}$,
        ...
        \begin{remark}[First-order matrix calculus]
            ...
            \begin{equation*}
                \begin{split}
                    \frac{\partial (\boldsymbol{y}- \boldsymbol{x}p)^{\top} \Omega
                    (\boldsymbol{y}- \boldsymbol{x}p)}{\partial p}&= -2 (\boldsymbol
                    {y}- \boldsymbol{x}p)^{\top} \Omega \boldsymbol{x}\\
                    &= -2 \boldsymbol{x}^{\top} \Omega (\boldsymbol{y}- \boldsymbol
                    {x}p) \\
                    \text{ with }\boldsymbol{A}= \boldsymbol{D}= - \boldsymbol{x}
                    \in \RR^{G}, \quad \boldsymbol{b}=&\boldsymbol{e}= \boldsymbol
                    {y}, \quad \boldsymbol{C}= \boldsymbol{\Omega}\text{ symmetric}
                \end{split}
            \end{equation*}
        \end{remark}
    ...
    \section{Special cases of the generative model}
        ...
        \begin{equation}
            ...
            \begin{array}{lcl}
                \ell(\boldsymbol{p}|\boldsymbol{y}, \boldsymbol{X}, \Sigma) & = & \sum_{j=1}^J \mathbb{P}_{p_j}(\mathrm{y}_{ G_j}|\mathrm{x}_{G_j}, \Sigma_j)                                                                                                                                 \\
                                                                            & = & C + \sum_{j=1}^J \left[ - |G_j| \log(p_j) - \frac{1}{2p_{j}^{2}} (\mathrm{y}_{ G_j}- \mathrm{x}_{G_j} p_j) \Omega_{j} (\mathrm{y}_{ G_j} - \mathrm{x}_{G_j} p_j)^\top \right] \, \text{(a)}                 \\
                                                                            & = & C + \sum_{j=1}^J \left[ - |G_j| \log(p_j) - \frac{1}{2p_{j}^{2}} \sum_{i=g}^{G_j} \Omega_{gg}\epsilon_{gg}^2 - \frac{1}{p_{j}^{2}} \sum_{1\le g <l \le |G_j|} \Omega_{gl} \epsilon_{gl}^2\right] \text{(b)} \\
            \end{array}
        \end{equation}

        with $C= \frac{1}{2}\sum_{j=1}^{J} \log(\det(\Omega_{j})) + \frac{1}{2}GJ
        ...
        l$ and $\Omega_{gl}$ input of the precision matrix indexed by row index $g$
        and column index $l$.
        ...
        \begin{equation}
            ...
            \begin{array}{ccl}
                \frac{\partial\ell(\boldsymbol{p}|\boldsymbol{y}, \boldsymbol{X}, \Sigma)}{\partial p_{j}}                                                                                                                                                                                                                                                  & =                   & 0 \\
                                                                                                                                                                                                                                                                                                                                                            & \Longleftrightarrow &   \\
                - \frac{|G_{j}|}{p_{j}} + \frac{(\mathrm{y}_{ G_j}- \mathrm{x}_{G_j}p_{j})^{\top} \Omega_{j}(\mathrm{y}_{ G_j}- \mathrm{x}_{G_j}p_{j})}{p_{j}^{3}} + \frac{x_{G_j}^{\top} \Omega_{j} (\mathrm{y}_{ G_j}- \mathrm{x}_{G_j}p_{j})}{p_{j}^{2}}                                                                                                 & =                   & 0 \\
                                                                                                                                                                                                                                                                                                                                                            & \Longleftrightarrow &   \\
                - |G_j| + \frac{\mathrm{y}_{ G_j}^{\top} \Omega_{j}\mathrm{y}_{ G_j}}{p_{j}^{2}} - 2 \frac{\mathrm{x}_{G_j}^{\top} \Omega_{j}\mathrm{y}_{G_j}}{p_{j}} + \cancel{\mathrm{x}_{G_j}^\top \Omega_{j} \mathrm{x}_{G_j}} + \frac{x_{G_j}^{\top} \Omega_{j} \mathrm{y}_{ G_j}}{p_{j}} - \cancel{\mathrm{x}_{G_j}^\top \Omega_{j} \mathrm{x}_{G_j}} & =                   & 0 \\
                                                                                                                                                                                                                                                                                                                                                            & \Longleftrightarrow &   \\
                - |G_j| p_j^2 - \mathrm{x}_{G_j}^\top \Omega_j \mathrm{y}_{G_j} p_j + \mathrm{y}_{ G_j}^\top \Omega_{j} \mathrm{y}_{ G_j}                                                                                                                                                                                                                   & =                   & 0
            \end{array}
        \end{equation}
        ...
        \begin{equation}
            ...
            \Omega_{j} \mathrm{y}_{ G_j}\pm
            \sqrt{\left( \boldsymbol{x}_{G_j}^\top \Omega_j \mathrm{y}_{ G_j}\right)^2
            +4 |G_j| \mathrm{y}_{ G_j}^\top \Omega_{j} \mathrm{y}_{ G_j} }}{2 |G_{j}|}
        \end{equation}
        Indeed, $\Delta=\left( \boldsymbol{x}_{G_j}^{\top} \Omega_{j} \mathrm{y}_{
        G_j}\right)^{2} +4 |G_{j}| \mathrm{y}_{ G_j}^{\top} \Omega_{j}\mathrm{y}_{
        ...
        \begin{equation}
            ...
            G_j}^{\top} \Omega_{j}\mathrm{y}_{ G_j}}{p_{j}^{4}}+ 2 \frac{x_{G_j}^{\top}
            \Omega_{j} \mathrm{y}_{ G_j}}{p_{j}^{3}}
        \end{equation}
        ...
        \begin{equation}
            ...
            \Omega_{j} \mathrm{y}_{ G_j}\pm
            \sqrt{\left( \boldsymbol{x}_{G_j}^\top \Omega_j \mathrm{y}_{ G_j}\right)^2
            + 3 |G_j| \mathrm{y}_{ G_j}^\top \Omega_{j} \mathrm{y}_{ G_j}}}{|G_{j}|}
        \end{equation}
        ...
        $\Omega=\mathrm{diag}(w_{1},\ldots,w_{G})$. DWLS supplies dampened gene
        ...
        \begin{theorem}[Generalised Least Squares Regression]
            ...
            \begin{equation*}
                ...
                =(\boldsymbol{\mu}^{\top}\Omega\boldsymbol{\mu})^{-1}
                \boldsymbol{\mu}^{\top}\Omega\boldsymbol{y}_{\cdot i},
            \end{equation*}
            where $\Omega=\Sigma^{-1}$. The estimator is unbiased, consistent,
            ...
            $\Omega$; off-diagonal residual correlations are then ignored and
