...
\begin{document}
    ...
    random rather than fixed. The closest published method is DSection
    \cite{erkkilaProbabilisticAnalysisGene2010}, which implements a
    \emph{univariate} Gaussian convolution (independent genes, gene-specific
    variances). DeMix \cite{ahnDeMixDeconvolutionMixed2013}, DeMixT
    \cite{wangTranscriptomeDeconvolutionHeterogeneous2018} and ISOpureR
    \cite{anghelISOpureRImplementationComputational2015} likewise treat
    tumour or stromal profiles as latent rather than observed; Ogundijo and
    Wang \cite{ogundijoSequentialMonteCarlo2017} extend DSection by
    approximating the posterior with sequential Monte Carlo. DeCovarT
    generalises the DSection likelihood to a \emph{multivariate} Gaussian
    convolution with a sparse cell-type-specific covariance
