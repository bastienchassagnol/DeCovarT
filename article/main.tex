...
\begin{document}
    ...
    $\boldsymbol{\zeta}=(\boldsymbol{\mu}, \boldsymbol{\Sigma}), \quad
    \boldsymbol{\mu}=(\boldsymbol{\mu}_{.j})_{j \in \widetilde{J}}\in
    \mathcal{M}_{G \times J}, \quad
    \boldsymbol{\Sigma}\in \mathcal{M}_{G \times G}$
    ...
    \begin{figure}[t]
        ...
        \caption[DAG models for linear and DeCovarT deconvolution]{%
            ...
            $\boldsymbol{y}_{gi\,|\,\cdot}\sim\mathcal{N}(\sum_{j}x_{gj}p_{ji},
            \sigma_{i}^{2})$.
            \textbf{b}~Unconstrained DeCovarT convolution
            $\boldsymbol{y}_{i\,|\,\cdot}\sim\mathcal{N}_{G}(\boldsymbol{\mu}
            \boldsymbol{p}_{i},\sum_{j}p_{ji}^{2}\boldsymbol{\Sigma}_{j})$ with
            $\boldsymbol{x}_{j}\sim\mathcal{N}_{G}(\boldsymbol{\mu}_{j},
            \boldsymbol{\Sigma}_{j})$.
            \textbf{c}~Constrained DeCovarT: unconstrained coordinates
            $\boldsymbol{\theta}_{i}\in\mathbb{R}^{J-1}$ are mapped to the open
            simplex by the $C^{2}$-diffeomorphism
            $\boldsymbol{\psi}:\boldsymbol{\theta}\mapsto\boldsymbol{p}$ with
            $p_{j}=e^{\theta_{j}}/(\sum_{k<J}e^{\theta_{k}}+1)$ for $j<J$,
            $p_{J}=1/(\sum_{k<J}e^{\theta_{k}}+1)$, and inverse
            $\boldsymbol{\psi}^{-1}(\boldsymbol{p})=
            \bigl(\ln(p_{j}/p_{J})\bigr)_{j<J}$
            (Eq.~\ref{eq-mapping-function}).
            For identifiability we treat variability as arising from the stochastic
            covariates $\boldsymbol{x}_{j}$.%
        }
        \label{fig:DAG-model}
    \end{figure}
